...
describe('LocalStorageService', () => {
  ...
  // Note: fc.jsonValue() is filtered to exclude -0 (including nested) because
  // JSON.stringify(-0) === "0", meaning -0 cannot survive a JSON round-trip.
  function hasNegZero(v: unknown): boolean {
    if (typeof v === 'number') return Object.is(v, -0);
    if (Array.isArray(v)) return v.some(hasNegZero);
    if (v !== null && typeof v === 'object') return Object.values(v).some(hasNegZero);
    return false;
  }
  const jsonValueNoNegZero = fc.jsonValue().filter((v) => !hasNegZero(v));
