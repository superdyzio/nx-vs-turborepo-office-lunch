...
describe('initDb', () => {
  ...
  it('should clear all ol_ keys before seeding', () => {
    ...
    // ol_votes and ol_orders should be gone (replaced or cleared)
    // The non-ol_ key should still be there
    expect(localStorage.getItem('non_ol_key')).toBe('should remain');
    // ol_votes should not contain old data
