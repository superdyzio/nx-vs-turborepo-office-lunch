 ...
 * For any TypeScript file in turborepo/packages/auth/src/, the file SHALL NOT
 * contain a relative import path that traverses outside the package boundary
 * (turborepo/packages/auth/).
 */

const WORKSPACE_ROOT = path.resolve(__dirname, '../../../../..');
const PACKAGE_ROOT = path.resolve(WORKSPACE_ROOT, 'turborepo/packages/auth');
...
function crossesBoundary(importingFile: string, importSpecifier: string): boolean {
  ...
  return (
    !normalisedResolved.startsWith(normalisedPackageRoot + path.sep) &&
    normalisedResolved !== normalisedPackageRoot
  );
}

describe('Property 1: No cross-boundary relative imports in auth', () => {
      ...
      fc.property(fc.constantFrom(...tsFiles), (filePath: string) => {
        ...
        for (const importSpecifier of relativeImports) {
          if (crossesBoundary(filePath, importSpecifier)) {
            ...
            throw new Error(
              `Cross-boundary import detected in "${relFile}": "${importSpecifier}" resolves outside turborepo/packages/auth/`
            );
          }
        }
